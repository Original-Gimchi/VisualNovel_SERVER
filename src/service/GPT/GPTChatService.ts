...
const GPTChatService = async (gPTChatServiceDto: GPTChatServiceDto) => {
    ...
    try {
        const openai = new OpenAIApi(configuration);
            ...
            let questionString = gPTChatServiceDto.quest + "이게 자소서 작성 문항이고," + gPTChatServiceDto.experience + "이 경험을 엮어줬으면 좋겠어 그리고" + gPTChatServiceDto.max + "자에 가깝게 글을 작성해주면 좋겠어."
        ...
        const response = await openai.createChatCompletion({
            model: "gpt-3.5-turbo", messages: [
                ...
                {role: "user", content: questionString + "마지막으로 자소서 제일 위에 대괄호로 감싼 소제목을 작성하고, 문항에 대한 답을 작성해줘"},
            ],
        }).data;
